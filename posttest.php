  ...
  <div class="content-wrapper">
    ...
    <div class="content" style="padding: 0px 20px 20px 20px;">
      <div class="card mb-0" style="border-radius: 25px; padding:15px 15px 15px 15px;">
        <div class="card-body">
          <h4 style="color: #1D5D9B; padding-bottom:10px;">Soal Posttest</h4>
          <div class="container-fluid">
            <div class="row">
              <?php
                $no = 0;
                $data = mysqli_query($conn, "SELECT * FROM kategori_kuis INNER JOIN guru ON kategori_kuis.id_guru = guru.id_guru WHERE kategori_kuis.nama_kategori = 'Post Test'");
                  ...
                  <div class="row">
                    <div class="col-12">
                      <div class="card-body" style="padding: 0px;">
                        <h5 class="card-title font-weight-bold" style="color:#1D5D9B">Mengenai Posttest</h5>
                        <p class="card-text text-justify"><?php echo $row['keterangan']?> 
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div class="card card-success" style="background-color: #CAF4FF; border-radius: 25px;">
                <div class="card-body">
                  <div class="row">
                    ...
                    ?>
                    <?php
                        $id_kuis = $row['id_kategori'];
                        // Cek apakah siswa sudah menyelesaikan kuis ini
                        $sql_check = "SELECT * FROM hasil_kuis WHERE id_siswa = ? AND id_kuis = ?";
                        $stmt_check = $conn->prepare($sql_check);
                        $stmt_check->bind_param("ii", $id_siswa, $id_kuis);
                        $stmt_check->execute();
                        $result_check = $stmt_check->get_result();
                        $is_completed = ($result_check->num_rows > 0);
                    ?>
                    <?php if ($is_completed): ?>
                    <a href="hasil_kuis.php?id=<?php echo $row['id_kategori']?>" class="btn btn-primary" style="background-color: #1D5D9B; color: white; border: none; width: 100%; height: 40px; border-radius: 10px;">Lihat Hasil</a>
                    <?php else: ?>
                    <a href="kuis.php?id=<?php echo $row['id_kategori']?> " class="btn btn-primary"style="background-color: #1D5D9B; color: white; border: none; width: 100%; height: 40px; border-radius: 10px;">Tekan untuk akses soal Posttest</a>
                    <?php endif; ?>
                    <?php
                        }
                    ?>
              ...
              <div class="card card-success" style="background-color: #CAF4FF; border-radius: 25px;">
                  <div class="card-body">
                    <div class="row">
                      <div class="col-12">
                        <div class="card-body" style="padding: 0px;">
                          <h5 class="card-title font-weight-bold" style="color:#1D5D9B">Konsep Pemrograman Berorientasi Objek</h5>
                          <p class="card-text text-justify">Capaian Pembelajaran yang diharapkan peserta didik mampu melakukan pemrograman terstruktur dan pemrograman 
                            berorientasi objek tingkat lanjut, menunjukkan dasar permodelan perangkat lunak berorientasi objek dengan 
                            memahami konsep, menerapkan alur kerja sistem, menunjukkan model, menerapkan relasi antarkelas, menerapkan 
                            interaksi antarobjek, menerapkan objek multimedia dalam aplikasi dengan menunjukkan aplikasi yang dapat 
                            menampilkan gambar, audio, dan video, melakukan pemrograman antarmuka grafis (Graphical User Interface) 
                            dengan memanfaatkan pustaka (library) pada proyek yang lebih kompleks melalui interpretasi model perangkat 
                            lunak kolaboratif pada proyek pengembangan perangkat lunak.
                          </p>
                        </div>
                      </div>
                    </div>
