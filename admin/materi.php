    ...
    <div class="content" style="padding: 0px 20px 20px 20px;">
      <div class="card mb-0" style="border-radius: 25px; padding:15px 15px 15px 15px;">
        <div class="card-body">
          <div class="container-fluid">
            <div class="row">
              <div class="d-grid gap-2 d-md-flex justify-content-md-end" style="padding-bottom: 15px; padding-right: 0px;">
                <a href="tambah-materi.php" class="btn btn-sm btn-outline-primary" style="background-color: #1D5D9B; color: white; border: none; width: 195px; height: 40px; border-radius: 10px; display: flex; justify-content: center; align-items: center;">Tambah Materi</a>
              </div><br>
                <?php
                $no = 1;
                $search_query = $search ? "WHERE materi.judul LIKE '%$search%'" : "";
                $data = mysqli_query($conn, "SELECT id_materi, materi.gambar, materi.judul, guru.nama FROM materi INNER JOIN guru ON materi.id_guru = guru.id_guru $search_query");
                            while($row = mysqli_fetch_array($data)){
                ?>
                <div class="card mb-3" style="max-width: fluid; padding: 15px;">
                  <div class="row g-0">
                    <div class="col-md-4">
                    <?php echo "<img src='../img/$row[gambar]' class='img-thumbnail' />";?>
                    </div>
                    <div class="col-md-6">
                      <div class="card-body">
                        <h3><?php echo $row['judul'];?></h3>
                        <p><?php echo $row['nama'];?></p>
                        <a href="detail-materi.php?detail=<?php echo $row['id_materi']?>" class="btn btn-sm btn-outline-primary">Detail</a>
                        <a href="edit-materi.php?edit=<?php echo $row['id_materi']?>" class="btn btn-sm btn-outline-secondary">Edit</a>
                        <a href="hapus-materi.php?hapus=<?php echo $row['id_materi']?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Apakah anda yakin akan menghapus data ini?')">Delete</a>  
                      </div>
                    </div>
                  </div>
                </div>
